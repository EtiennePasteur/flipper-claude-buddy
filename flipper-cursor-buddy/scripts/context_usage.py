...
SOCKET_PATH = os.environ.get("FLIPPER_SOCKET", "/tmp/cursor-flipper-bridge.sock")
STATE_PATH = os.environ.get("FLIPPER_CONTEXT_STATE", "/tmp/cursor-flipper-context.json")
...
def extract_usage(hook_input: dict[str, Any]) -> dict[str, int | None]:
    ...
    if isinstance(info, dict):
        if out["context_pct"] is None:
            ...
            out["context_pct"] = _context_pct_from_tokens(total, info.get("model_context_window"))
        if out["session_pct"] is None:
            out["session_pct"] = _session_pct_from_rate_limits(info.get("rate_limits") or {})

    if out["session_pct"] is None:
        out["session_pct"] = _session_pct_from_rate_limits(hook_input.get("rate_limits") or {})
