...
# Cursor sessionStart hook: ensure bridge is running, then notify Flipper.
...
PLUGIN_DATA="${PLUGIN_DATA:-${CLAUDE_PLUGIN_DATA:-/tmp/flipper-cursor-buddy}}"
...
# Cursor hooks are configured through FLIPPER_* environment variables.
...
# Pass current project directory so the bridge can discover Cursor skills.
...
if [ -S "$SOCKET" ] && [ -f "$PIDFILE" ]; then
    ...
    if [ -n "$OLD_PID" ] && kill -0 "$OLD_PID" 2>/dev/null && [ "$INSTALLED_HASH" != "$CURRENT_HASH" ]; then
        ...
        rm -f "$SOCKET" "$PIDFILE" "/tmp/cursor-flipper-bridge.refcount"
        OLD_PID=""
    fi
...
# Register the current Cursor session before the bridge reports it connected.
...
PROJECT_DIR="$(pwd)"
echo "{\"action\":\"cursor_connect\",\"project_dir\":\"$PROJECT_DIR\"}" \
...
echo "{\"action\":\"notify\",\"sound\":\"connect\",\"vibro\":true,\"text\":\"Cursor\",\"subtext\":\"$SUBTEXT_DISPLAY\"}" \
    | nc -U "$SOCKET" 2>/dev/null || true
