...
def main():
    if sys.version_info < (3, 10):
        sys.exit(
            f"flipper-cursor-buddy requires Python 3.10 or higher. "
            f"You are running Python {sys.version.split()[0]}. "
            f"Please upgrade Python, then reinstall the plugin:\n"
            f"Reinstall the flipper-cursor-buddy plugin after upgrading Python."
        )

    parser = argparse.ArgumentParser(description="Flipper Cursor Buddy bridge")
    parser.add_argument(
        "--transport",
