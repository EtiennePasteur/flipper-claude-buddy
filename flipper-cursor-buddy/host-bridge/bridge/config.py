...
SOCKET_PATH = os.environ.get(
    "FLIPPER_BRIDGE_SOCKET", "/tmp/cursor-flipper-bridge.sock"
)

# Host identity reported to the Flipper header label (claude/codex/cursor)
HOST_TYPE = os.environ.get("FLIPPER_HOST_TYPE", "cursor")
...
# Persistent bridge state used by the Cursor bridge wrappers.
_state_dir = _plugin_data or os.path.join(tempfile.gettempdir(), "flipper-cursor-buddy")
BRIDGE_ENABLED_FILE = os.environ.get(
    "FLIPPER_BRIDGE_ENABLED_FILE",
    os.path.join(_state_dir, "bridge-enabled"),
)


def read_bridge_enabled(default: bool = True) -> bool:
    try:
        raw = Path(BRIDGE_ENABLED_FILE).read_text(encoding="utf-8").strip().lower()
    except Exception:
        return default
...
def write_bridge_enabled(enabled: bool) -> None:
    path = Path(BRIDGE_ENABLED_FILE)
...
# Project root for scanning Cursor skills and custom commands.
