"""Main daemon: routes messages between Flipper serial and Cursor hooks IPC."""
...
class Daemon:
    ...
    async def _handle_flipper_msg(self, msg: dict):
        ...
        if msg_type == "hello":
            ...
            await self.serial.send(
                protocol.notify_msg("ready", vibro=True, text="Cursor", subtext="Connected")
            )
            await self._mark_flipper_session_connected()
            # Send command menu
            ...
            await self._send_to_cursor("yes")
            ...
            if not self._menu_sent:
                ...
                if commands:
                    ...
                    log.debug("Sending menu (%d commands, %d bytes)", len(commands), len(menu_bytes))
                    await self.serial.send(menu_bytes)
                await self._mark_flipper_session_connected(play_connect=True)
    ...
    async def _handle_ipc_action(self, request: dict) -> dict:
        ...
        if action == "notify":
            if self._perm_future and not self._perm_future.done():
                log.info("Dismissing pending permission (deferring to Cursor)")
            ...
            config.write_bridge_enabled(True)
            ...
            try:
                ...
                if result is None:
                    ...
                    return {"status": "no_flipper"}
                if result.get("ask"):
                    log.info("Permission dismissed — deferring to Cursor")
                    return {"status": "ask"}
    ...
    BUILTIN_COMMANDS = [
        "/help", "/models", "/rules", "/mcp", "/skills", "/status",
    ]

    def _load_commands(self) -> list[str]:
        """Load built-in commands and user/project Cursor skills.

        Sources (checked in both ``~/.cursor/`` and ``<project>/.cursor/``):
        1. ``commands/*.md``  — filename (without .md) becomes ``/<name>``
        2. ``skills/<name>/SKILL.md`` — ``name`` frontmatter becomes ``/<name>``
        3. ``flipper-commands.txt`` for optional manual menu additions
        """
        commands: set[str] = set(self.BUILTIN_COMMANDS)

        home_cursor = Path.home() / ".cursor"
        project_cursor = Path(config.PROJECT_DIR) / ".cursor"
        roots = [home_cursor, project_cursor]
        log.debug("Command discovery: home=%s project=%s", home_cursor, project_cursor)
        ...
        # Optional manual menu additions.
    ...
    async def _send_to_cursor(self, text: str):
    ...
    async def run(self):
        ...
        try:
            ...
            # bridge process (spawned when the user starts a new Codex
