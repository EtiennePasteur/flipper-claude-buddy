"""Main daemon: routes messages between Flipper serial and Codex hooks IPC."""
...
class Daemon:
    ...
    async def _handle_ipc_action(self, request: dict) -> dict:
        ...
        if action == "notify":
            if self._perm_future and not self._perm_future.done():
                log.info("Dismissing pending permission (deferring to Codex)")
            ...
            try:
                ...
                if result is None:
                    ...
                    return {"status": "no_flipper"}
                if result.get("ask"):
                    log.info("Permission dismissed — deferring to Codex")
                    return {"status": "ask"}
    ...
    BUILTIN_COMMANDS = [
        "/compact", "/diff", "/feedback", "/goal", "/help", "/hooks", "/init",
        "/mcp", "/model", "/permissions", "/plan", "/review", "/skills", "/status",
    ]

    def _load_commands(self) -> list[str]:
        """Load built-in commands and user/project Codex skills.

        Sources (checked in both ``~/.codex/`` and ``<project>/.codex/``):
        ...
        project_codex = Path(config.PROJECT_DIR) / ".codex"
        roots = [home_codex, project_codex]
        log.debug("Command discovery: home=%s project=%s", home_codex, project_codex)
    ...
    async def _send_to_codex(self, text: str):
